...
# UK vignette regression test -------------------------------------------------
#
# Source this script after loading DemoKin, then run:
#
#   run_kin_multi_stage_time_variant_2sex_with_death_uk_test()
#
# The test checks that the death-augmented implementation returns the same
# living kin as kin_multi_stage_time_variant_2sex(). It uses the UK parity
# data from vignettes/2_2_TwoSex_TimeVarying_AgeStage.Rmd, restricted to one
# year so it is a fast time-constant demonstration.
run_kin_multi_stage_time_variant_2sex_with_death_uk_test <- function() {
  requireNamespace("Matrix")
  data("Female_parity_fert_list_UK", envir = environment())
  data("Parity_transfers_by_age_list_UK", envir = environment())
  data("Female_parity_mortality_list_UK", envir = environment())
  data("Male_parity_mortality_list_UK", envir = environment())

  output_year <- 1965

  # Match the UK parity example in the vignette: fertility and parity
  # transitions are androgynous, while mortality is sex-specific.
  U_list_females <- Female_parity_mortality_list_UK[1]
  U_list_males <- Male_parity_mortality_list_UK[1]
  F_list_females <- Female_parity_fert_list_UK[1]
  F_list_males <- Female_parity_fert_list_UK[1]
  T_list_females <- Parity_transfers_by_age_list_UK[1]
  T_list_males <- Parity_transfers_by_age_list_UK[1]

  old <- kin_multi_stage_time_variant_2sex(
    U_list_females = U_list_females,
    U_list_males = U_list_males,
    F_list_females = F_list_females,
    F_list_males = F_list_males,
    T_list_females = T_list_females,
    T_list_males = T_list_males,
    birth_female = 1 - 0.51,
    parity = TRUE,
    output_kin = c("d", "m", "ys"),
    summary_kin = FALSE,
    ...
    output_years = output_year
  )

  new_full <- kin_multi_stage_time_variant_2sex_with_death(
    U_list_females = U_list_females,
    U_list_males = U_list_males,
    F_list_females = F_list_females,
    F_list_males = F_list_males,
  ...
  head(old)
  head(new_full)
  old |> summarise(sum(living), .by = c(kin, stage_kin))
  new_full |> summarise(sum(living), sum(dead, na.rm = T), .by = c(kin, stage_kin))


  key <- c("age_focal", "age_kin", "stage_kin", "sex_kin", "year", "kin", "cohort")
  old <- old[do.call(order, old[key]), c(key, "living")]
  new <- new_full[do.call(order, new_full[key]), c(key, "living")]

  stopifnot(isTRUE(all.equal(old, new, check.attributes = FALSE, tolerance = 1e-10)))
  invisible(new_full)
}

run_kin_multi_stage_time_variant_2sex_with_death_toy_test <-
  run_kin_multi_stage_time_variant_2sex_with_death_uk_test
...
#' @export
#'
kin_multi_stage_time_variant_2sex_with_death <- function(U_list_females = NULL,
                                              U_list_males = NULL,
                                              F_list_females = NULL,
                                              F_list_males = NULL,
                                              T_list_females = NULL,
  ...
  for(year in 1:no_years){
    ...
    if(year == 1){
      ...
      kin_out_1 <- all_kin_dy_with_death(U_tilde_females,
                              U_tilde_males ,
                              F_tilde_females,
                              F_tilde_males,
                              1-birth_female[1],
    ...
    kin_out <- all_kin_dy_TV_with_death(U_tilde_females,
                             U_tilde_males,
                             F_tilde_females,
                             F_tilde_males,
                             1-birth_female[year],
  ...
  kin_full <- create_full_dists_df_with_death(relative_data,
                                   relative_names,
                                   output_years,
                                   output_years[1],
                                   na,
                                   ...
                                   output_kin)
  if(summary_kin){
    kin_summary <- create_cumsum_df_with_death(relative_data,
                                relative_names,
                                output_years,
                                output_years[1],
                                na,
...
#' nrow = 4* na * ns (living/dead by 2-sex age-stage structure) and ncol = na (Focal's age)
#' yielding the age*stage distribution of kin for each age of Focal

all_kin_dy_with_death <- function(Uf,
                       Um,
                       Ff,
                       Fm,
  ...
  # Death-change: augment the living age-stage-sex projection with an
  # absorbing dead block, following the logic used in kin_multi_stage().
  # Birth matrices have zeros in the dead block, so dead kin do not reproduce.
  living_n <- 2*n
  state_n <- 2*living_n
  z <- Matrix::Matrix(nrow = living_n, ncol = living_n, data = 0, sparse = TRUE)
  I <- Matrix::Diagonal(living_n)
  Mproj <- Matrix::Diagonal(n = living_n, x = as.numeric(1 - Matrix::colSums(Uproj)))
  Uproj <- rbind(cbind(Uproj, z), cbind(Mproj, I))
  Fproj <- rbind(cbind(Fproj, z), cbind(z, z))
  Fprojstar <- rbind(cbind(Fprojstar, z), cbind(z, z))
  G_tilde <- rbind(cbind(G_tilde, z), cbind(z, z))

  X_Focal  <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  IC_Focal <- rep(0, state_n)
  if(sex_Focal == "Female"){
  ...
  X_children <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_grand_children <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_great_grand_children <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_parents <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_grand_parents <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_great_grand_parents <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_older_sibs  <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_younger_sibs  <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_older_niece_nephew  <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_younger_niece_nephew  <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_older_aunt_uncle  <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_younger_aunts_uncles <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_older_cousins  <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  X_younger_cousins  <- Matrix::Matrix(nrow = state_n, ncol = na, data = 0, sparse = TRUE)
  ...
  X_Focal[,1] <- IC_Focal
  X_parents[seq_len(living_n), 1] <- parents_joint_age_stage
...
#' yielding the age*stage distribution of kin for each age of Focal
#'
all_kin_dy_TV_with_death <- function(Uf,
                          Um,
                          Ff,
                          Fm,
                          alpha, ## alpha = sex ratio male:female (i.e., 1 - birth_female)
  ...
  # Death-change: carry forward dead kin in an absorbing block. The population
  # structure above stays living-only because it is used for time-varying
  # parenting/mixing distributions.
  living_n <- 2*n
  state_n <- 2*living_n
  z <- Matrix::Matrix(nrow = living_n, ncol = living_n, data = 0, sparse = TRUE)
  I <- Matrix::Diagonal(living_n)
...
create_kin_df_with_death <- function(kin_matrix_lists,
                                     kin_names,
                                     years,
                                     start_year,
                                     na,
                                     ...
                                     summary_kin = FALSE) {
  if (length(years) > 1) {
    year_step <- min(diff(sort(unique(years))))
  } else {
    year_step <- 1
  }
  years_needed <- c(years, max(years) + year_step)
  years_available <- start_year + seq_len(length(kin_matrix_lists[[1]])) - 1
  years_needed <- intersect(years_needed, years_available)

  df_year_list <- list()
  for (j in years_needed) {
    ii <- as.numeric(j) - start_year + 1
    df_list <- list()
    for (i in seq_along(kin_names)) {
      kin_member <- kin_names[[i]]
      kin_data <- kin_matrix_lists[[i]][[ii]]
      df <- as.data.frame(as.matrix(kin_data))
      dims <- dim(kin_data)
      nr <- dims[1]
      nc <- dims[2]
      ...
      if (nr == 2 * n_sex) {
        # Backward-compatible path for the replicated living-only matrices.
        female_living <- df[seq_len(n_sex), seq_len(nc)]
        male_living <- df[(n_sex + 1):(2 * n_sex), seq_len(nc)]
        both_kin <- rbind(
          add_kin_state_labels(female_living, na, ns, "f", "living"),
          add_kin_state_labels(male_living, na, ns, "m", "living")
        )
      } else if (nr == 4 * n_sex) {
        # Death-change: rows are living female, living male, dead female,
        # dead male. Dead rows are the absorbing stock generated by Uproj.
        female_living <- df[seq_len(n_sex), seq_len(nc)]
        male_living <- df[(n_sex + 1):(2 * n_sex), seq_len(nc)]
        female_dead <- df[(2 * n_sex + 1):(3 * n_sex), seq_len(nc)]
        male_dead <- df[(3 * n_sex + 1):(4 * n_sex), seq_len(nc)]
        both_kin <- rbind(
          add_kin_state_labels(female_living, na, ns, "f", "living"),
          add_kin_state_labels(male_living, na, ns, "m", "living"),
          add_kin_state_labels(female_dead, na, ns, "f", "dead"),
          add_kin_state_labels(male_dead, na, ns, "m", "dead")
        )
      } else {
        stop("Unexpected kin matrix row count.")
      }

      id_vars <- c("age", "stage", "Sex", "alive")
      both_kin <- reshape2::melt(both_kin, id = id_vars)
      if (summary_kin) {
        both_kin <- both_kin %>%
          dplyr::group_by(variable, stage, Sex, alive) %>%
          dplyr::summarise(value = sum(value), .groups = "drop")
      }

      both_kin <- both_kin %>%
        dplyr::transmute(
          age_focal = as.numeric(gsub("[^0-9.-]", "", variable)) - 1,
          age_kin = if (summary_kin) NA_real_ else age,
          stage_kin = as.factor(stage),
          sex_kin = Sex,
          alive = alive,
          count = value,
      ...
      df_list[[length(df_list) + 1]] <- both_kin
    }
    df_year_list[[length(df_year_list) + 1]] <- do.call("rbind", df_list)
  }

  kin_long <- do.call("rbind", df_year_list)

  # Death-change: deaths are created by projecting from t to t + 1 and from
  # Focal age a to a + 1, so relocate the dead stock back to the exposure
  # period and Focal age, as in kin_multi_stage() and the time-variant death
  # relocation proposal.
  living <- kin_long[kin_long$alive == "living", ]
  dead <- kin_long[kin_long$alive == "dead", ]
  dead$year <- dead$year - year_step
  dead$age_focal <- dead$age_focal - 1

  key <- c("year", "kin", "age_focal", "stage_kin", "sex_kin")
  if (!summary_kin) {
    key <- c(key, "age_kin")
  }

  living <- living[living$year %in% years, c(key, "count")]
  names(living)[names(living) == "count"] <- "living"
  dead <- dead[dead$year %in% years & dead$age_focal >= 0, c(key, "count")]
  names(dead)[names(dead) == "count"] <- "dead"

  out <- merge(living, dead, by = key, all.x = TRUE)
  out$cohort <- as.numeric(out$year) - as.numeric(out$age_focal)
  if (summary_kin) {
    out$age_kin <- NULL
  }
  if (!is.null(specific_kin)) {
    out <- out %>% dplyr::filter(kin %in% specific_kin)
  }
  out
}

add_kin_state_labels <- function(df, na, ns, sex, alive) {
  df$stage <- rep(seq_len(ns), na)
  df$age <- rep(seq(0, na - 1), each = ns)
  df$Sex <- sex
  df$alive <- alive
  df
}

create_cumsum_df_with_death <- function(kin_matrix_lists,
                                        kin_names,
                                        years,
                                        start_year,
                                        na,
                                        ns,
                                        specific_kin = NULL) {
  create_kin_df_with_death(
    kin_matrix_lists = kin_matrix_lists,
    kin_names = kin_names,
    years = years,
    start_year = start_year,
    na = na,
    ns = ns,
    specific_kin = specific_kin,
    summary_kin = TRUE
  )
}

create_full_dists_df_with_death <- function(kin_matrix_lists,
                                            kin_names,
                                            years,
                                            start_year,
                                            na,
